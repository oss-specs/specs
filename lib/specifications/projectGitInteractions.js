...
function generateRepoMetaData(projectData, sha, tagsAndBranches, currentBranchName, files) {
    ...
    if (missingValues.length) {
        //throw new Error("Porject data is missing values for: " + missingValues);
...
function getRefInformation(projectData, targetBranchName) {
    ...
    return getBranch(projectData, targetBranchName)
        ...
        .then(getAllFiles)
        .then(function(files) {
            listOfFiles = files.map(function(f) {
                return {
                    ref: branch.name(),
                    featureRoute: url.resolve('project/', projectData.repoName + "/" + f),
                    featureName: f,
                    featureNameShort: f.replace(/\.feature$/, '') }
            });
