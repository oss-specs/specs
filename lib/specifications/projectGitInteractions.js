...
function clone(projectData, cloneOptions) {
    ...
    if (!projectData.repoUrl) {
      throw new TypeError('Clone called without a repo URL.');
    }
...
function update(projectData) {
    var repository;
    if (!projectData.currentBranchName) {
      throw new TypeError('Update called without a current branch name.');
    }
