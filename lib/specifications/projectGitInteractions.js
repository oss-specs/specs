...
function generateRepoMetaData(projectData, sha, tagsAndBranches, currentBranchName, files) {
    ...
    if (missingValues.length) {
        //throw new Error("Project data is missing values for: " + missingValues);
...
function update(projectData) {
    ...
    return git.Repository.openBare(projectData.localPath)
        ...
        .then(function (branch) {
            return getRefInformation(projectData, branch.name());
        });
}
...
function getRefInformation(projectData, targetBranchName) {
    ...
    return getBranch(projectData, targetBranchName)
        .then(function(ref) {
            currentRef = ref;
            shortBranchName = currentRef.shorthand()
            return git.Repository.open(projectData.localPath)
        })
        .then(function (repo) {
            repository = repo;
        ...
        .then(function(files) {
            listOfFiles = files.map(function(fileOfInterestName) {
                return {
                    ref: currentRef.name(),
                    featureRoute: url.resolve('project/', projectData.repoName + "/" + fileOfInterestName),
        ...
        .then(function (refs) {
            ...
            return generateRepoMetaData(projectData, headCommit, tagsAndBranches, currentRef.name(), listOfFiles);
        });
}
